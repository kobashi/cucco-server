...
管理トークンを入力して「接続」すると使える。画面は3つのタブに分かれる。

**卓タブ** — 卓の一覧が自動更新(既定5秒。停止・間隔変更可)で表示される。
各行から**詳細**(観戦者視点の状態。手札は含まれない)・**中止**(進行中の
ゲームを強制終了して卓を閉じる)・**削除**(ゲームの走っていない卓を削除)を
実行できる。

**成績タブ** — 「成績を読み込む」で、プレイヤー別の成績・内蔵AI方策別の
成績・直近のゲーム結果・評価モードの実行一覧を表示する(CLIの
`cucco.tools.stats`と同じ集計。読み取り専用なので稼働中でも安全)。
上部のプルダウンで**集計期間**を切り替えられる(§6.5)。
**表示名には実名が含まれることがある**ため、画面共有や保存に注意する。

**メンテナンスタブ** — §8 を参照。保存状況(ゲーム記録数・DBサイズ・行動ログ
の件数と容量)の確認と、古いデータの削除ができる。
...
**1. ゲームが終わった時点で人間が誰もいない卓は、その場で閉じる。** 進行中だった
ゲームは最後まで実行して成績も記録し、その直後に閉じる(内蔵AIのタスクも止まる)。
「観戦者がタブを閉じたAI卓」はほぼこれで片付き、ログには
`closed table <卓ID>: its game ended with no human present` が出る。**連戦は主催者が
明示的に開始しない限り始まらない**ので、無人の卓が次のゲームを配ることはない。

**2. 放置卓の自動掃除(GC)。** 上で拾えない卓 — 作ったまま誰も来なかった卓、
ゲームが始まる前に全員が去った待機中の卓、クラッシュして残った卓 — を、既定で
60秒ごとのスイープで削除する。手動のabort/removeは即時掃除として併用できる。
...
だけの卓は常時ブロードキャストしていて無操作時間が増えない**ため。

> [!NOTE]
> GCによる削除は進行中のゲームを途中で打ち切る(その対局は記録されない)。
> 経路1がゲームの切れ目で閉じるのと対照的なので、**同じ「掃除」でも記録の
> 残り方が違う**点に注意。

### 6.4 成績の確認(CLI)

管理コンソールの成績タブと同じ集計をコマンドラインからも見られる。
読み取り専用なので稼働中に実行してよい:

```bash
python -m cucco.tools.stats                  # プレイヤー別の成績(開催中の期間)
...
python -m cucco.tools.stats --periods        # 集計期間の一覧(ID・期間名・対局数)
python -m cucco.tools.stats --period 1       # 〆済みの期間を見返す
python -m cucco.tools.stats --period all     # 全期間の通算
```

期間を指定しない場合は**開催中の期間**を集計する(§6.5)。出力の1行目に
どの期間を集計しているかが表示される。

### 6.5 集計期間(戦績のリセット)

成績は**集計期間**ごとに区切って集計する。ゼミの回や学期の区切りで「〆る」と、
そこまでの成績が締め切られ、次の期間が空から始まる。

**〆かた** — 管理コンソールの成績タブ下部「集計期間の管理」で、次の期間名
(任意。空欄なら`第N期`)を入れて「開催中の集計を〆る」。CLIからも:

```bash
python -m cucco.tools.admin --token XXXX periods                    # 期間の一覧
python -m cucco.tools.admin --token XXXX close-period --name 2026年度後期
```

> [!IMPORTANT]
> **〆ても記録は消えません。** 締めた期間の対局記録はそのまま残り、コンソールの
> プルダウンや`--period <ID>`でいつでも見返せます。成績表が新しい期間の分だけを
> 数えるようになるだけです。記録そのものを消したい場合は§8.1の削除機能を使います
> (こちらは取り消せません)。

**既存データの扱い** — 期間管理の導入前に記録された対局は、サーバーの初回起動時に
`第0期`(実際の記録範囲を開始・終了とする締め済みの期間)としてまとめて取り込まれ、
新しい期間が空から始まる。取り込みは一度だけで、再起動しても重複しない。
...
そのまま使える)。集計期間の導入時は、初回起動で既存の対局がすべて`第0期`に
取り込まれ、成績タブの既定表示が空の新しい期間になる(§6.5)。過去の成績は
プルダウンで`第0期`を選べば従来どおり見られる。
...
- 行動ログは1ゲーム1ファイルで増え続ける。連戦が自動で進まなくなったぶん増え方は
  穏やかになったが、人が見ている限りAI卓は何度でも回せるので、下記の掃除機能で
  定期的に削減する
- バックアップはサーバー停止中に`data/`をコピーするのが最も確実

### 8.1 保存状況の確認と掃除(管理コンソールのメンテナンスタブ)

「保存状況を読み込む」で、ゲーム記録数・参加記録数・評価モード記録数・
DBサイズ・行動ログの件数と容量・最古/最新の記録日時・開催中の集計期間を
表示する。

> [!TIP]
> **期間で区切りたいだけなら削除ではなく§6.5の「〆る」を使ってください。**
> 〆れば成績表は新しい期間の分だけになり、それまでの記録も残ります。
> 以下は記録そのものを消す操作です。

削除は2系統あり、それぞれ「N日より古いものだけ」と「全件」を選べる:

| 操作 | 対象 |
|---|---|
| 成績の削除 | `results.db`のゲーム・参加・評価モードの記録(削除後にVACUUMでファイルサイズも縮小)。**全件削除の場合は集計期間の履歴も消え、新しい`第1期`から始まる** |
| 行動ログの削除 | `data/action_logs/*.jsonl`(更新時刻で判定) |

> [!CAUTION]
> **削除は取り消せません。** 実行前に`data/`のバックアップを取ってください。
> 全件削除は確認ダイアログに「削除」と入力しないと実行されません(日数指定の
> 削除は通常の確認ダイアログ)。実行した削除はサーバーログにも記録されます。

成績と行動ログは独立して削除できます。**ディスクを食うのはほぼ行動ログ**
なので、成績(研究データ)は残したまま行動ログだけ削る運用が実用的です。
...
| 卓が消えずに残る | ゲーム終了時に人間が残っていた卓は待機状態で残る(仕様)。誰もいない待機卓はGCが10分で掃除する。急ぐならコンソールの中止/削除 |
| AI卓が1ゲームで止まる | 仕様。連戦は主催者が「続けて遊ぶ」→開始を押したときだけ始まる |
...
**セッションの後**
- [ ] 成績を確認(管理コンソールの成績タブ、または`cucco.tools.stats`)。
      **出力はリポジトリにコミットしない**
- [ ] 長期停止するなら安全に停止(§5)、`data/`のバックアップ
- [ ] 行動ログが増えすぎていないか(メンテナンスタブの保存状況。§8.1)

**ゼミの回・学期の区切りで**
- [ ] 成績を確認・記録してから集計期間を**〆る**(§6.5)。〆ると次の期間が空から
      始まる。締めた期間の記録は消えず、あとから`--period <ID>`で見返せる
