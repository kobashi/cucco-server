...
- **人間向けブラウザクライアント(2種)**: ビルドツール不要の素のHTML/CSS/
  ...
  卓を囲むレイアウト+カード/チップのアニメーションで実際のゲームプレイに
  近づけたプレイ用クライアント([`clients/play/`](clients/play/))の2つを
  同一サーバー・同一卓で相互に利用できる
...
クライアント・人間向けブラウザクライアント2種(リファレンス+プレイ用)まで
実装済み、テスト253件全てパス。複数回のレビュー(Fable5)を経て主要な
不具合は修正済み。GitHub Pages + Cloudflare Tunnelでの学外公開運用まで
実績あり。成績・統計の閲覧UIは今後の課題。
