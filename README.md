...
- **サンプルクライアント**: すぐ動かせるMock AI(3方策)と対話式Stubクライアント
  ([`clients/`](clients/))
- **人間向けブラウザクライアント**: ビルドツール不要の素のHTML/CSS/ES Modules。
  卓の作成・参加者募集から対局・観戦まで一通り動作する
  ([`clients/web/`](clients/web/))

## 状態

ドメイン層・プロトコル層・サーバー層・永続化層・評価モード・サンプルクライアント・
人間向けブラウザクライアントまで実装済み、テスト206件全てパス。複数回のレビュー
(Fable5)を経て主要な不具合は修正済み。成績・統計の閲覧UIは今後の課題。
